...
# Supplementary analyses: adding BMI (BMI z-score) as covariate in analyses:
# Age and PDS on probability of gradient flip (binary variable - logistic) 
...
resdir = '/data/p_02667/development/results/supplementary/BMI/'
resdir_fig = '/data/p_02667/development/results/supplementary/BMI/figures/'
...
covar_df <- abcd_demo_long %>%
  ...
  dplyr::select(src_subject_id_fmt, age_months, pds_p_score, sex, tot_SA, BMIz, family_id, site_id, eventname) %>% 
...
covar_df$BMIz <- as.numeric(covar_df$BMIz)
...
newdata_age <- expand.grid(
  ...
  BMIz = mean(covar_df$BMIz, na.rm = TRUE),  # hold BMIz at mean
