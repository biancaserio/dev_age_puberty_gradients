# Script for postprocessing ABCD fMRIprep **baseline** files using XCP-D, which also outputs FC matrices, based on input list from a txt file 
# Original Version (ran baseline - worked)
# Date (OG) creation: 22.11.2024, Author: Bianca Serio
# Date amended: 10.04.2025
# # added check "if sub exists in XCP-D output folder, skip it"
# # added dropping subdataset and then getting it again before cd'ing into datalad subject folder
...
# 5. run script: bash get_ABCD_data_fmriprep_xcp-d_fc_specifiedsubs_droppingfirst.sh /data/p_02667/development/scripts/sub_3_timepoints_qc_val_relabelled.txt
...
subject_list_file="$1"
failed_subjects_file="/data/pt_02667/data/ABCD/XCP-D_output/logs/failed_subjects_get_fmriprep.txt"
missing_files_log="/data/pt_02667/data/ABCD/XCP-D_output/logs/missing_files_log.txt"
...
pptDIR="/data/pt_02667/data/ABCD/ABCD_fMRIprep/fmriprep/"

# Log directory base path
log_base="/data/pt_02667/data/ABCD/XCP-D_output/logs"
...
for sub in "${subs_data_to_download[@]}"; do
    ...
    xcpd_output_dir="/data/pt_02667/data/ABCD/XCP-D_output/${sub}"
    ...
    if [ -d "$subject_dir" ]; then
    	...
    	# Drop the subject data before processing
        echo "Dropping subject data for $sub to start fresh..."
        datalad drop --what datasets --reckless kill -r -d "$subject_dir"
        ...
        if cd "$subject_dir"; then
            ...
            datalad get --no-data .
            datalad get -J 10 sub-*.html ses-baselineYear1Arm1/func/* ses-baselineYear1Arm1/anat/*desc-preproc_T1w* ses-baselineYear1Arm1/anat/*desc-brain_mask* ses-baselineYear1Arm1/anat/*from-MNI152NLin6Asym_to-T1w_mode-image_xfm.h5 ses-baselineYear1Arm1/anat/*from-T1w_to-MNI152NLin6Asym_mode-image_xfm.h5
            
            # Get additional files required by XCP-D pipeline that are not available in fmriprep output
            cd ses-baselineYear1Arm1/func/ || { 
            ...
            mask_file=$(ls "${sub}_ses-baselineYear1Arm1"*desc-brain_mask.nii.gz 2>/dev/null | head -n 1 | sed 's/"//g' | sed 's/@$//')
            ...
            transform_file=$(ls "${sub}_ses-baselineYear1Arm1"*from-T1w_to-MNI152NLin6Asym_mode-image_xfm.h5 2>/dev/null | head -n 1 | sed 's/"//g' | sed 's/@$//')
            ...
            # Drop the downloaded data except .html files (in /ses-baselineYear1Arm1) and confound files (in /func)
            datalad drop --what filecontent --reckless kill -J 10 ses-baselineYear1Arm1/anat/* ses-baselineYear1Arm1/func/*_AROMA* ses-baselineYear1Arm1/func/*_boldref* ses-baselineYear1Arm1/func/*_desc-brain* ses-baselineYear1Arm1/func/*_desc-MELODIC* ses-baselineYear1Arm1/func/*_desc-preproc* ses-baselineYear1Arm1/func/*_from* ses-baselineYear1Arm1/func/*_hemi* ses-baselineYear1Arm1/func/*_space* || { echo "Error dropping files for $sub"; }         
