...
# Computes total surface area by extracting data output of script p3_area2mgh.sh (surface area- available data in /pt_02667/ directory at BASELINE) and summing across hemispheres
...
path_list_baseline_SA = os.listdir(datadir+'ABCD_baseline_SA')
path_list_baseline_SA.sort()  # this contains html symlinks as well as other files that are not subject files on top

    
sub_list_baseline_SA = []  # contains all subjects with SA output


for sub in path_list_baseline_SA:
    ...
    if sub.startswith("sub-") and "html" not in sub:
        sub_list_baseline_SA.append(sub)
...
for sub in sub_list_baseline_SA:

    counter += 1

    print(f"Subject {counter} out of {len(sub_list_baseline_SA)}")

    sub_path_to_SA_data = datadir+'ABCD_baseline_SA/'+sub
    ...
    # Append to list
    total_SA_baseline.append(tot_sa)
...
print(f"----- Exporting as csv at {datadir}ABCD_total_SA_baseline.csv -----")

# Create a DataFrame
df_tot_SA_baseline = pd.DataFrame({"src_subject_id_fmt": sub_list_baseline_SA, "tot_SA_baseline": total_SA_baseline})

# Export
df_tot_SA_baseline.to_csv(datadir+'ABCD_total_SA_baseline.csv', header = True, index = False)


print(f"Number of subjects with Total SA output: {len(sub_list_baseline_SA)}")
print(f"Are there any values of total SA that are NaN? -> {pd.isna(total_SA_baseline).any()}")
