...
# Computes total surface area by extracting data output of script p3_area2mgh.sh (surface area- available data in /pt_02667/ directory at 2Y FOLLOW-UP) and summing across hemispheres
...
print("----- Sorting through available data in fu2y directory -----")


# fetch data available in directory post datalad download
path_list_fu2y_SA = os.listdir(datadir+'ABCD_fu2y_SA')
path_list_fu2y_SA.sort()  # this contains html symlinks as well as other files that are not subject files on top

    
sub_list_fu2y_SA = []  # contains all subjects with SA output


for sub in path_list_fu2y_SA:
...
for sub in sub_list_fu2y_SA:

    counter += 1

    print(f"Subject {counter} out of {len(sub_list_fu2y_SA)}")

    sub_path_to_SA_data = datadir+'ABCD_fu2y_SA/'+sub
...
print(f"----- Exporting as csv at {datadir}ABCD_total_SA_fu2y.csv -----")

# Create a DataFrame
df_tot_SA_fu2y = pd.DataFrame({"src_subject_id_fmt": sub_list_fu2y_SA, "tot_SA_fu2y": total_SA_fu2y})

# Export
df_tot_SA_fu2y.to_csv(datadir+'ABCD_total_SA_fu2y.csv', header = True, index = False)


print(f"Number of subjects with Total SA output: {len(sub_list_fu2y_SA)}")
print(f"Are there any values of total SA that are NaN? -> {pd.isna(total_SA_fu2y).any()}")
