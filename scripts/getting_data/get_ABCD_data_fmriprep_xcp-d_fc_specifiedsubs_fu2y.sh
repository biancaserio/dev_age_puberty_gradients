# Script for postprocessing ABCD fMRIprep **2y follow-up** files using XCP-D, which also outputs FC matrices, based on input list from a txt file
# Copied from original version to tweak to make specific to 2y follow-up
# Date creation: 31.01.2025, Author: Bianca Serio
...
# 5. run script: bash get_ABCD_data_fmriprep_xcp-d_fc_specifiedsubs_fu2y.sh /data/p_02667/scripts/sub_3_timepoints_qc_val_relabelled.txt
...
subject_list_file="$1"
failed_subjects_file="/data/pt_02667/data/ABCD/XCP-D_output_fu2y/logs/failed_subjects_get_fmriprep.txt"
missing_files_log="/data/pt_02667/data/ABCD/XCP-D_output_fu2y/logs/missing_files_log.txt"
...
pptDIR="/data/pt_02667/data/ABCD/ABCD_fu2y_fmriprep22.1.1_FSready/fmriprep/"

# Log directory base path
log_base="/data/pt_02667/data/ABCD/XCP-D_output_fu2y/logs"

# Eventname variable
eventname="ses-2YearFollowUpYArm1"
...
for sub in "${subs_data_to_download[@]}"; do
    ...
    xcpd_output_dir="/data/pt_02667/data/ABCD/XCP-D_output_fu2y/${sub}"
    ...
    if [ -d "$subject_dir" ]; then
        ...
        if cd "$subject_dir"; then
            ...
            if [ -z "$fsLR_bold_files" ]; then
            	...
            	/data/pt_02667/data/ABCD/ABCD_fu2y_fmriprep22.1.1_FSready/fmriprep \
            	/data/pt_02667/data/ABCD/XCP-D_output_fu2y \
            	participant \
            	--mode 'none' \
            	--participant-label "${sub#sub-}" \
            	--bids-filter-file /data/pt_02667/data/ABCD/bianca/bids_filter_file_fu2y.json \
            	--nprocs 36 \
            	--input-type 'fmriprep' \
            	--file-format 'cifti' \
            	--dummy-scans 'auto' \
