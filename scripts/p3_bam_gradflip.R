...
# Main analyses: Age and PDS on probability of gradient flip (binary variable - logistic) 
...
names(mat_SA_axis_NOT_aligned_baseline)  # print contents of matfile
...
dim(list_SA_axis_grad_num_baseline)
dim(sub_ID_baseline)
...
dim(list_SA_axis_grad_num_fu2y)
dim(sub_ID_fu2y)
...
dim(list_SA_axis_grad_num_fu4y)
dim(sub_ID_fu4y)
...
### Concatenate lists of S-A axis gradient numbers and demo dataframes for longitudinal analyses

# S-A axis gradient numbers
...
#### Making grad_flip binary variable

### Distribution/counts of S-A axis gradient numbers

# Plot histogram
hist(list_SA_axis_grad_num_long,
     breaks = 30,
     col = "skyblue",
     border = "white",
...
table(list_SA_axis_grad_num_long)
...
# Split grad_flip using the same indices
grad_flip_M <- grad_flip[male_idx]
grad_flip_F <- grad_flip[female_idx]
...
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# RUN BAM on gradient flip
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------


##### Main Model grad_flip ~ Sex + s(PDS) + s(age) + tot_SA + random effect(site/family relatedness/subject)
...
summary_bam_main_gflip <- summary(bam_main_gflip)

# Save summary
capture.output(summary_bam_main_gflip, file = file.path(resdir, "summary_bam_main_gflip.txt"))

# Check for adequate basis dimension and model fit
mgcv::k.check(bam_main_gflip)
...
df_age <- smooth_estimates(bam_main_gflip, select = "s(age)")
df_pds <- smooth_estimates(bam_main_gflip, select = "s(pds)")
...
ggsave(paste(resdir_fig, 'bam_smes_main_age_on_gradflip.svg', sep = ''), plot = plot_smes_age, width = 5, height = 5, dpi = 300)
ggsave(paste(resdir_fig, 'bam_smes_main_pds_on_gradflip.svg', sep = ''), plot = plot_smes_pds, width = 5, height = 5, dpi = 300)
...
ggsave(paste(resdir_fig, 'bam_smes_panel_main_ageEpds_on_gradflip.svg', sep = ''), plot = plot_smes_panel, width = 11, height = 5, dpi = 300)
...
ggsave(paste(resdir_fig, 'bam_margtraj_main_age_on_gradflip.svg', sep = ''), plot = plot_margtraj_age, width = 6, height = 5, dpi = 300)
...
ggsave(paste(resdir_fig, 'bam_margtraj_main_pds_on_gradflip.svg', sep = ''), plot = plot_margtraj_pds, width = 6, height = 5, dpi = 300)
...
ggsave(paste(resdir_fig, 'bam_margtraj_panel_main_ageEpds_on_gradflip.svg', sep = ''), plot = plot_margtraj_panel, width = 11, height = 5, dpi = 300)
...
summary_bam_gflip_age.pds_M = summary(bam_gflip_age.pds_M)

# Save summary
capture.output(summary_bam_gflip_age.pds_M, file = file.path(resdir, "summary_bam_gflip_age.pds_M.txt"))

# Plot smooths
draw(bam_gflip_age.pds_M, select = "ti(age,pds)")

# Check for adequate basis dimension and model fit 
mgcv::k.check(bam_gflip_age.pds_M)
...
summary_bam_gflip_age.pds_F = summary(bam_gflip_age.pds_F)

# Save summary
capture.output(summary_bam_gflip_age.pds_F, file = file.path(resdir, "summary_bam_gflip_age.pds_F.txt"))

# Plot smooths
draw(bam_gflip_age.pds_F, select = "ti(age,pds)")

# Check for adequate basis dimension and model fit 
mgcv::k.check(bam_gflip_age.pds_F)
...
summary_bam_gflip_sex.pds = summary(bam_gflip_sex.pds)

# Save summary
capture.output(summary_bam_gflip_sex.pds, file = file.path(resdir, "summary_bam_gflip_sex.pds.txt"))

# Plot smooths
draw(bam_gflip_sex.pds, 
     select = c("s(pds):sexF", "s(pds):sexM"), 
     residuals = TRUE)

# Check for adequate basis dimension and model fit 
mgcv::k.check(bam_gflip_sex.pds)
...
summary_bam_gflip_sex.age = summary(bam_gflip_sex.age)

# Save summary
capture.output(summary_bam_gflip_sex.age, file = file.path(resdir, "summary_bam_gflip_sex.age.txt"))

# Plot smooths
draw(bam_gflip_sex.age, 
     select = c("s(age):sexF", "s(age):sexM"), 
     residuals = TRUE)

# Check for adequate basis dimension and model fit 
mgcv::k.check(bam_gflip_sex.age)



### FOLLOW UP:
# Given that age*sex interaction appears statistically significant (even though age main effect is not):
# Trying to test for age effects seaprately by sex:
# s(age, by = sex, k = 10, fx = FALSE) (not including main effects): separate smooth for each sex -> testing main effect separately by sex

bam_gflip_sex.age_v2 <- bam(
  grad_flip ~ 
    sex +
    #s(age, k = 10, fx = FALSE) +  # default
    s(pds, k = 10, fx = FALSE) +  # default
    s(age, by = sex, k = 10, fx = FALSE) + # default, one line output for significance of interaction
    tot_SA +
    s(site_id, bs = "re") +
    s(site.family, bs = "re") +
    s(site.family.subject, bs = "re"),
...
summary_bam_gflip_sex.age_v2 = summary(bam_gflip_sex.age_v2)

# Save summary
capture.output(summary_bam_gflip_sex.age_v2, file = file.path(resdir, "summary_bam_gflip_sex.age_v2.txt"))

# Plot smooths
draw(bam_gflip_sex.age_v2, 
     select = c("s(age):sexF", "s(age):sexM"), 
     residuals = TRUE)

# Check for adequate basis dimension and model fit 
mgcv::k.check(bam_gflip_sex.age_v2)




#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# RUN Mixed Effects Logistic Regression on r gradient flip
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
# Fit GLMM with binary DV and nested random effects
glmer_main_gflip <- glmer(
  grad_flip ~ sex + pds_z + age_z + tot_SA_z + (1 | site_id/family_id/subject_id),
  data = covar_df,
  family = binomial(link = "logit"),
  control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))
)

summary_glmer_main_gflip = summary(glmer_main_gflip)

# Save summary
capture.output(summary_glmer_main_gflip, file = file.path(resdir, "summary_glmer_main_gflip.txt"))

#exp(fixef(glmer_main_gflip))  # Odds ratios
