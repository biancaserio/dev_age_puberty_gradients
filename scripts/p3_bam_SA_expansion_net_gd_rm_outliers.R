...
resdir = '/data/p_02667/development/results/network_dispersion/'
resdir_fig = '/data/p_02667/development/results/network_dispersion/figures/'
resdir_gd = '/data/p_02667/development/results/gd/'
resdir_gd_fig = '/data/p_02667/development/results/gd/figures/'
...
#mean_gd_top10_baseline <- mat_SA_expansion_metrics_baseline$mean.gd.top10.baseline  # shape: 400, 3950

# short
mean_gd_short_node_baseline <- mat_SA_expansion_metrics_baseline$mean.gd.short.node.baseline  # shape: 1, 3950
mean_gd_short_node_baseline <- t(mean_gd_short_node_baseline)  #transposing to get the shape: 3950, 1

# medium
mean_gd_medium_node_baseline <- mat_SA_expansion_metrics_baseline$mean.gd.medium.node.baseline  # shape: 1, 3950
mean_gd_medium_node_baseline <- t(mean_gd_medium_node_baseline)  #transposing to get the shape: 3950, 1

# long
mean_gd_long_node_baseline <- mat_SA_expansion_metrics_baseline$mean.gd.long.node.baseline  # shape: 1, 3950
mean_gd_long_node_baseline <- t(mean_gd_long_node_baseline)  #transposing to get the shape: 3950, 1
...
#mean_gd_top10_fu2y <- mat_SA_expansion_metrics_fu2y$mean.gd.top10.fu2y  # shape: 1253, 400

# short
mean_gd_short_node_fu2y <- mat_SA_expansion_metrics_fu2y$mean.gd.short.node.fu2y  # shape: 1, 1253
mean_gd_short_node_fu2y <- t(mean_gd_short_node_fu2y)  #transposing to get the shape: 1253, 1

# medium
mean_gd_medium_node_fu2y <- mat_SA_expansion_metrics_fu2y$mean.gd.medium.node.fu2y  # shape: 1, 1253
mean_gd_medium_node_fu2y <- t(mean_gd_medium_node_fu2y)  #transposing to get the shape: 1253, 1

# long
mean_gd_long_node_fu2y <- mat_SA_expansion_metrics_fu2y$mean.gd.long.node.fu2y  # shape: 1, 1253
mean_gd_long_node_fu2y <- t(mean_gd_long_node_fu2y)  #transposing to get the shape: 1253, 1
...
#mean_gd_top10_fu4y <- mat_SA_expansion_metrics_fu4y$mean.gd.top10.fu4y  # shape: 906, 400

# short
mean_gd_short_node_fu4y <- mat_SA_expansion_metrics_fu4y$mean.gd.short.node.fu4y  # shape: 1, 906
mean_gd_short_node_fu4y <- t(mean_gd_short_node_fu4y)  #transposing to get the shape: 906, 1

# medium
mean_gd_medium_node_fu4y <- mat_SA_expansion_metrics_fu4y$mean.gd.medium.node.fu4y  # shape: 1, 906
mean_gd_medium_node_fu4y <- t(mean_gd_medium_node_fu4y)  #transposing to get the shape: 906, 1

# long
mean_gd_long_node_fu4y <- mat_SA_expansion_metrics_fu4y$mean.gd.long.node.fu4y  # shape: 1, 906
mean_gd_long_node_fu4y <- t(mean_gd_long_node_fu4y)  #transposing to get the shape: 906, 1
...
# by short/medium/long (by node)
mean_gd_short_node = c(mean_gd_short_node_baseline, mean_gd_short_node_fu2y, mean_gd_short_node_fu4y)
mean_gd_medium_node = c(mean_gd_medium_node_baseline, mean_gd_medium_node_fu2y, mean_gd_medium_node_fu4y)
mean_gd_long_node = c(mean_gd_long_node_baseline, mean_gd_long_node_fu2y, mean_gd_long_node_fu4y)

mean(mean_gd_short_node)
mean(mean_gd_medium_node)
mean(mean_gd_long_node)
...
summary_bam_SA_exp_leg1_sex.pds_rm_outliers = summary(bam_SA_exp_leg1_sex.pds)

# Save summary
capture.output(summary_bam_SA_exp_leg1_sex.pds_rm_outliers, file = file.path(resdir, "summary_bam_SA_exp_leg1_sex.pds_rm_outliers.txt"))

# Check for adequate basis dimension and model fit
mgcv::k.check(bam_SA_exp_leg1_sex.pds)



##### Model including sex*age interaction

bam_SA_exp_leg1_sex.age <- bam(
  SA_expansion_net_long ~
    sex_ord +
    s(age, k = 10, fx = FALSE) +  # default
    s(pds, k = 10, fx = FALSE) +  # default
...
summary_bam_SA_exp_leg1_sex.age_rm_outliers = summary(bam_SA_exp_leg1_sex.age)

# Save summary
capture.output(summary_bam_SA_exp_leg1_sex.age_rm_outliers, file = file.path(resdir, "summary_bam_SA_exp_leg1_sex.age_rm_outliers.txt"))

# Check for adequate basis dimension and model fit
mgcv::k.check(bam_SA_exp_leg1_sex.age)
...
newdata_age <- expand.grid(
  age = age_grid,
  sex = levels(covar_df$sex),
  SA_expansion_net_long = mean(SA_expansion_net_long, na.rm = TRUE),  # hold SAexp at mean
  pds = mean(covar_df$pds, na.rm = TRUE),  # hold PDS at mean
  tot_SA = mean(covar_df$tot_SA, na.rm = TRUE),  # hold total SA at mean
  site_id = covar_df$site_id[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family = covar_df$site.family[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
...
##### rcoef: Model including sex*SAexp interaction

bam_SA_exp_leg2_rcoef_sex.SAexp <- bam(
  list_SA_axis_corr_to_ref_long_z ~
    sex_ord +
    s(age, k = 10, fx = FALSE) +  # default
    s(pds, k = 10, fx = FALSE) +  # default
    s(SA_expansion_net_long, k = 10, fx = FALSE) +  # default
    s(SA_expansion_net_long, by = sex_ord, k = 10, fx = FALSE) + # default, one line output for significance of interaction
    tot_SA +
    s(site_id, bs = "re") +
    s(site.family, bs = "re") +
    s(site.family.subject, bs = "re"),
...
summary_bam_SA_exp_leg2_rcoef_sex.SAexp_rm_outliers = summary(bam_SA_exp_leg2_rcoef_sex.SAexp)

# Save summary
capture.output(summary_bam_SA_exp_leg2_rcoef_sex.SAexp_rm_outliers, file = file.path(resdir, "summary_bam_SA_exp_leg2_rcoef_sex.SAexp_rm_outliers.txt"))

# Check for adequate basis dimension and model fit
mgcv::k.check(bam_SA_exp_leg2_rcoef_sex.SAexp)
...
##### gradflip: Model including sex*SAexp interaction


bam_SA_exp_leg2_gradflip_sex.SAexp <- bam(
  grad_flip ~ 
    sex_ord +
    s(age, k = 10, fx = FALSE) +  # default (including age because -although it wasnt a significant predictor in gradflip main analysis- it was for M)
    s(pds, k = 10, fx = FALSE) +  # default
...
summary_bam_SA_exp_leg2_gradflip_sex.SAexp_rm_outliers = summary(bam_SA_exp_leg2_gradflip_sex.SAexp)

# Save summary
capture.output(summary_bam_SA_exp_leg2_gradflip_sex.SAexp_rm_outliers, file = file.path(resdir, "summary_bam_SA_exp_leg2_gradflip_sex.SAexp_rm_outliers.txt"))

# Check for adequate basis dimension and model fit
mgcv::k.check(bam_SA_exp_leg2_gradflip_sex.SAexp)
...
# Save summary
capture.output(summary_bam_GD_V, file = file.path(resdir_gd, "summary_bam_GD_V_rm_outliers.txt"))
...
# Save summary
capture.output(summary_bam_GD_SM, file = file.path(resdir_gd, "summary_bam_GD_SM_rm_outliers.txt"))
...
# Save summary
capture.output(summary_bam_GD_DA, file = file.path(resdir_gd, "summary_bam_GD_DA_rm_outliers.txt"))
...
# Save summary
capture.output(summary_bam_GD_VA, file = file.path(resdir_gd, "summary_bam_GD_VA_rm_outliers.txt"))
...
# Save summary
capture.output(summary_bam_GD_L, file = file.path(resdir_gd, "summary_bam_GD_L_rm_outliers.txt"))
...
# Save summary
capture.output(summary_bam_GD_FP, file = file.path(resdir_gd, "summary_bam_GD_FP_rm_outliers.txt"))
...
# Save summary
capture.output(summary_bam_GD_DMN, file = file.path(resdir_gd, "summary_bam_GD_DMN_rm_outliers.txt"))
...
#ggsave(paste(resdir_gd_fig, 'bam_smes_age_on_GD_per_network_all_rm_outliers.svg', sep = ''), plot = plot_smes_age_all, width = 6, height = 5, dpi = 300)
#ggsave(paste(resdir_gd_fig, 'bam_smes_pds_on_GD_per_network_all_rm_outliers.svg', sep = ''), plot = plot_smes_pds_all, width = 6, height = 5, dpi = 300)
...
ggsave(paste(resdir_gd_fig, 'bam_smes_panel_ageEpds_on_GD_per_network_all_rm_outliers.svg', sep = ''), plot = plot_smes_panel, width = 11, height = 5, dpi = 300)
...
# before saving decide if you want with or without CIs (and remove or add "_CIs" accordingly)
#ggsave(paste(resdir_gd_fig, 'bam_smes_age_on_GD_per_network_all_sig_rm_outliers_CIs.svg', sep = ''), plot = plot_smes_age_all_sig, width = 6, height = 5, dpi = 300)
#ggsave(paste(resdir_gd_fig, 'bam_smes_pds_on_GD_per_network_all_sig_rm_outliers_CIs.svg', sep = ''), plot = plot_smes_pds_all_sig, width = 6, height = 5, dpi = 300)
...
ggsave(paste(resdir_gd_fig, 'bam_smes_panel_ageEpds_on_GD_per_network_all_sig_rm_outliers_CIs.svg', sep = ''), plot = plot_smes_panel_all_sig, width = 11, height = 5, dpi = 300)
...
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Plotting of age and pds effects (predicted marginal trajectories) on Mean GD -> one plot per network (and effect)
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
plot_margtraj_age <- ggplot(
  ...
  labs(x = "Age (in months)", y = "Mean length of functional connections\nin the visual network", color = "Sex", fill = "Sex") +
  theme_minimal(base_size = 14) +
  theme(
    axis.text = element_text(color = "black"),
    axis.title.y.right = element_text(angle = 90, vjust = 0.5)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_age_on_GD_V_rm_outliers.svg', sep = ''), plot = plot_margtraj_age, width = 6, height = 5, dpi = 300)
...
plot_margtraj_pds <- ggplot(
  ...
  labs(x = "Pubertal stage (PDS score)", y = "Mean length of functional connections\nin the visual network", color = "Sex", fill = "Sex") +
  theme_minimal(base_size = 14) +
  theme(
    axis.text = element_text(color = "black"),
    axis.title.y.right = element_text(angle = 90, vjust = 0.5)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_pds_on_GD_V_rm_outliers.svg', sep = ''), plot = plot_margtraj_pds, width = 6, height = 5, dpi = 300)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_age_on_GD_SM_rm_outliers.svg', sep = ''), plot = plot_margtraj_age, width = 6, height = 5, dpi = 300)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_pds_on_GD_SM_rm_outliers.svg', sep = ''), plot = plot_margtraj_pds, width = 6, height = 5, dpi = 300)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_age_on_GD_DA_rm_outliers.svg', sep = ''), plot = plot_margtraj_age, width = 6, height = 5, dpi = 300)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_pds_on_GD_DA_rm_outliers.svg', sep = ''), plot = plot_margtraj_pds, width = 6, height = 5, dpi = 300)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_age_on_GD_VA_rm_outliers.svg', sep = ''), plot = plot_margtraj_age, width = 6, height = 5, dpi = 300)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_pds_on_GD_VA_rm_outliers.svg', sep = ''), plot = plot_margtraj_pds, width = 6, height = 5, dpi = 300)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_age_on_GD_L_rm_outliers.svg', sep = ''), plot = plot_margtraj_age, width = 6, height = 5, dpi = 300)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_pds_on_GD_L_rm_outliers.svg', sep = ''), plot = plot_margtraj_pds, width = 6, height = 5, dpi = 300)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_age_on_GD_FP_rm_outliers.svg', sep = ''), plot = plot_margtraj_age, width = 6, height = 5, dpi = 300)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_pds_on_GD_FP_rm_outliers.svg', sep = ''), plot = plot_margtraj_pds, width = 6, height = 5, dpi = 300)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_age_on_GD_DMN_rm_outliers.svg', sep = ''), plot = plot_margtraj_age, width = 6, height = 5, dpi = 300)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_pds_on_GD_DMN_rm_outliers.svg', sep = ''), plot = plot_margtraj_pds, width = 6, height = 5, dpi = 300)
...
# Save summary
capture.output(summary_bam_SA_exp_GDs_all_net_rm_outliers, file = file.path(resdir_gd, "summary_bam_SA_exp_GDs_all_net_rm_outliers.txt"))
...
df_age <- smooth_estimates(bam_SA_exp_GDs_all_net, select = "s(age)")
df_pds <- smooth_estimates(bam_SA_exp_GDs_all_net, select = "s(pds)")
...
# before saving decide if you want with or without CIs (and remove or add "_CIs" accordingly)
ggsave(paste(resdir_gd_fig, 'bam_smes_all_networks_GDs_on_SAexp_rm_outliers_CIs.svg', sep = ''), plot = plot_smes_all_net, width = 6, height = 5, dpi = 300)
...
ggsave(paste(resdir_gd_fig, 'bam_smes_all_networks_age_on_SAexp_rm_outliers.svg', sep = ''), plot = plot_smes_age, width = 5, height = 5, dpi = 300)
ggsave(paste(resdir_gd_fig, 'bam_smes_all_networks_pds_on_SAexp_rm_outliers.svg', sep = ''), plot = plot_smes_pds, width = 5, height = 5, dpi = 300)
...
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Plotting of effects of Mean GDs by network (predicted marginal trajectories) on S-A axis expansion -> one plot per network
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
V_grid<- seq(
  min(V_mean_gd_long, na.rm=TRUE),
  max(V_mean_gd_long, na.rm=TRUE),
  length.out=200
)
...
newdata_V <- expand.grid(
  ...
  pds = mean(covar_df$pds, na.rm = TRUE),  # hold PDS at mean
  tot_SA = mean(covar_df$tot_SA, na.rm = TRUE),  # hold total SA at mean
  V_mean_gd_long = V_grid,  # these are the predictions that we're plotting
  SM_mean_gd_long = mean(SM_mean_gd_long, na.rm = TRUE),  # hold at mean
  DA_mean_gd_long = mean(DA_mean_gd_long, na.rm = TRUE),  # hold at mean
  VA_mean_gd_long = mean(VA_mean_gd_long, na.rm = TRUE),  # hold at mean
  L_mean_gd_long = mean(L_mean_gd_long, na.rm = TRUE),  # hold at mean
  FP_mean_gd_long = mean(FP_mean_gd_long, na.rm = TRUE),  # hold at mean
  DMN_mean_gd_long = mean(DMN_mean_gd_long, na.rm = TRUE),  # hold at mean
  site_id = covar_df$site_id[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family = covar_df$site.family[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family.subject = covar_df$site.family.subject[1]  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
)
...
plot_margtraj_V <- ggplot(
  smooth_preds_V, 
  aes(x = V_mean_gd_long, y = .fitted, color = sex, fill = sex)
) +
  geom_ribbon(
    aes(ymin = .lower_ci, ymax = .upper_ci),  # Use .lower and .upper (instead of manual +/- 2*se) in order to get the limits of the credible interval on the fitted values, on the specified scale
    alpha = 0.25, 
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_GD_V_on_SAexp_rm_outliers.svg', sep = ''), plot = plot_margtraj_V, width = 6, height = 5, dpi = 300)
...
SM_grid<- seq(
  min(SM_mean_gd_long, na.rm=TRUE),
  max(SM_mean_gd_long, na.rm=TRUE),
  length.out=200
)
...
newdata_SM <- expand.grid(
  ...
  pds = mean(covar_df$pds, na.rm = TRUE),  # hold PDS at mean
  tot_SA = mean(covar_df$tot_SA, na.rm = TRUE),  # hold total SA at mean
  V_mean_gd_long = mean(V_mean_gd_long, na.rm = TRUE),  # hold at mean
  SM_mean_gd_long = SM_grid,  # these are the predictions that we're plotting
  DA_mean_gd_long = mean(DA_mean_gd_long, na.rm = TRUE),  # hold at mean
  VA_mean_gd_long = mean(VA_mean_gd_long, na.rm = TRUE),  # hold at mean
  L_mean_gd_long = mean(L_mean_gd_long, na.rm = TRUE),  # hold at mean
  FP_mean_gd_long = mean(FP_mean_gd_long, na.rm = TRUE),  # hold at mean
...
plot_margtraj_SM <- ggplot(
  smooth_preds_SM, 
  aes(x = SM_mean_gd_long, y = .fitted, color = sex, fill = sex)
) +
  geom_ribbon(
    aes(ymin = .lower_ci, ymax = .upper_ci),  # Use .lower and .upper (instead of manual +/- 2*se) in order to get the limits of the credible interval on the fitted values, on the specified scale
    alpha = 0.25, 
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_GD_SM_on_SAexp_rm_outliers.svg', sep = ''), plot = plot_margtraj_SM, width = 6, height = 5, dpi = 300)
...
DA_grid<- seq(
  min(DA_mean_gd_long, na.rm=TRUE),
  max(DA_mean_gd_long, na.rm=TRUE),
  length.out=200
)
...
newdata_DA <- expand.grid(
  ...
  V_mean_gd_long = mean(V_mean_gd_long, na.rm = TRUE),  # hold at mean
  SM_mean_gd_long = mean(SM_mean_gd_long, na.rm = TRUE),  # hold at mean
  DA_mean_gd_long = DA_grid,  # these are the predictions that we're plotting
  VA_mean_gd_long = mean(VA_mean_gd_long, na.rm = TRUE),  # hold at mean
  L_mean_gd_long = mean(L_mean_gd_long, na.rm = TRUE),  # hold at mean
  FP_mean_gd_long = mean(FP_mean_gd_long, na.rm = TRUE),  # hold at mean
  DMN_mean_gd_long = mean(DMN_mean_gd_long, na.rm = TRUE),  # hold at mean
...
plot_margtraj_DA <- ggplot(
  smooth_preds_DA, 
  aes(x = DA_mean_gd_long, y = .fitted, color = sex, fill = sex)
) +
  geom_ribbon(
    aes(ymin = .lower_ci, ymax = .upper_ci),  # Use .lower and .upper (instead of manual +/- 2*se) in order to get the limits of the credible interval on the fitted values, on the specified scale
    alpha = 0.25, 
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_GD_DA_on_SAexp_rm_outliers.svg', sep = ''), plot = plot_margtraj_DA, width = 6, height = 5, dpi = 300)
...
VA_grid<- seq(
  min(VA_mean_gd_long, na.rm=TRUE),
  max(VA_mean_gd_long, na.rm=TRUE),
  length.out=200
)
...
newdata_VA <- expand.grid(
  ...
  SM_mean_gd_long = mean(SM_mean_gd_long, na.rm = TRUE),  # hold at mean
  DA_mean_gd_long = mean(DA_mean_gd_long, na.rm = TRUE),  # hold at mean
  VA_mean_gd_long = VA_grid,  # these are the predictions that we're plotting
  L_mean_gd_long = mean(L_mean_gd_long, na.rm = TRUE),  # hold at mean
  FP_mean_gd_long = mean(FP_mean_gd_long, na.rm = TRUE),  # hold at mean
  DMN_mean_gd_long = mean(DMN_mean_gd_long, na.rm = TRUE),  # hold at mean
  site_id = covar_df$site_id[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
...
plot_margtraj_VA <- ggplot(
  smooth_preds_VA, 
  aes(x = VA_mean_gd_long, y = .fitted, color = sex, fill = sex)
) +
  geom_ribbon(
    aes(ymin = .lower_ci, ymax = .upper_ci),  # Use .lower and .upper (instead of manual +/- 2*se) in order to get the limits of the credible interval on the fitted values, on the specified scale
    alpha = 0.25, 
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_GD_VA_on_SAexp_rm_outliers.svg', sep = ''), plot = plot_margtraj_VA, width = 6, height = 5, dpi = 300)
...
L_grid<- seq(
  min(L_mean_gd_long, na.rm=TRUE),
  max(L_mean_gd_long, na.rm=TRUE),
  length.out=200
)
...
newdata_L <- expand.grid(
  ...
  DA_mean_gd_long = mean(DA_mean_gd_long, na.rm = TRUE),  # hold at mean
  VA_mean_gd_long = mean(VA_mean_gd_long, na.rm = TRUE),  # hold at mean
  L_mean_gd_long = L_grid,  # these are the predictions that we're plotting
  FP_mean_gd_long = mean(FP_mean_gd_long, na.rm = TRUE),  # hold at mean
  DMN_mean_gd_long = mean(DMN_mean_gd_long, na.rm = TRUE),  # hold at mean
  site_id = covar_df$site_id[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family = covar_df$site.family[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
...
plot_margtraj_L <- ggplot(
  smooth_preds_L, 
  aes(x = L_mean_gd_long, y = .fitted, color = sex, fill = sex)
) +
  geom_ribbon(
    aes(ymin = .lower_ci, ymax = .upper_ci),  # Use .lower and .upper (instead of manual +/- 2*se) in order to get the limits of the credible interval on the fitted values, on the specified scale
    alpha = 0.25, 
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_GD_L_on_SAexp_rm_outliers.svg', sep = ''), plot = plot_margtraj_L, width = 6, height = 5, dpi = 300)
...
FP_grid<- seq(
  min(FP_mean_gd_long, na.rm=TRUE),
  max(FP_mean_gd_long, na.rm=TRUE),
  length.out=200
)
...
newdata_FP <- expand.grid(
  ...
  VA_mean_gd_long = mean(VA_mean_gd_long, na.rm = TRUE),  # hold at mean
  L_mean_gd_long = mean(L_mean_gd_long, na.rm = TRUE),  # hold at mean
  FP_mean_gd_long = FP_grid,  # these are the predictions that we're plotting
  DMN_mean_gd_long = mean(DMN_mean_gd_long, na.rm = TRUE),  # hold at mean
  site_id = covar_df$site_id[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family = covar_df$site.family[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family.subject = covar_df$site.family.subject[1]  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
...
plot_margtraj_FP <- ggplot(
  smooth_preds_FP, 
  aes(x = FP_mean_gd_long, y = .fitted, color = sex, fill = sex)
) +
  geom_ribbon(
    aes(ymin = .lower_ci, ymax = .upper_ci),  # Use .lower and .upper (instead of manual +/- 2*se) in order to get the limits of the credible interval on the fitted values, on the specified scale
    alpha = 0.25, 
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_GD_FP_on_SAexp_rm_outliers.svg', sep = ''), plot = plot_margtraj_FP, width = 6, height = 5, dpi = 300)
...
DMN_grid<- seq(
  min(DMN_mean_gd_long, na.rm=TRUE),
  max(DMN_mean_gd_long, na.rm=TRUE),
  length.out=200
)
...
newdata_DMN <- expand.grid(
  ...
  L_mean_gd_long = mean(L_mean_gd_long, na.rm = TRUE),  # hold at mean
  FP_mean_gd_long = mean(FP_mean_gd_long, na.rm = TRUE),  # hold at mean
  DMN_mean_gd_long = DMN_grid,   # these are the predictios that we're plotting
  site_id = covar_df$site_id[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family = covar_df$site.family[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family.subject = covar_df$site.family.subject[1]  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
)
...
plot_margtraj_DMN <- ggplot(
  smooth_preds_DMN, 
  aes(x = DMN_mean_gd_long, y = .fitted, color = sex, fill = sex)
) +
  geom_ribbon(
    aes(ymin = .lower_ci, ymax = .upper_ci),  # Use .lower and .upper (instead of manual +/- 2*se) in order to get the limits of the credible interval on the fitted values, on the specified scale
    alpha = 0.25, 
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_GD_DMN_on_SAexp_rm_outliers.svg', sep = ''), plot = plot_margtraj_DMN, width = 6, height = 5, dpi = 300)
...
# Save summary
capture.output(summary_bam_SA_exp_GDs_sml_net, file = file.path(resdir_gd, "summary_bam_SA_exp_GDs_sml_net_rm_outliers.txt"))
...
# Extract the data for more control
df_age <- smooth_estimates(bam_SA_exp_GDs_sml_net, select = "s(age)")
df_pds <- smooth_estimates(bam_SA_exp_GDs_sml_net, select = "s(pds)")
...
# before saving decide if you want with or without CIs (and remove or add "_CIs" accordingly)
ggsave(paste(resdir_gd_fig, 'bam_smes_sml_net_GDs_on_SAexp_rm_outliers_CIs.svg', sep = ''), plot = plot_smes_all_sml_net, width = 6, height = 5, dpi = 300)
...
# be careful at current CIs when plotting - only need to plot this with CIs
ggsave(paste(resdir_gd_fig, 'bam_smes_panel_allnetGDsEsml_net_GDs_on_SAexp_rm_outliers_CIs.svg', sep = ''), plot = plot_smes_panel_allnetGDs_sml, width = 11, height = 5, dpi = 300)
...
ggsave(paste(resdir_gd_fig, 'bam_smes_sml_net_age_on_SAexp_rm_outliers.svg', sep = ''), plot = plot_smes_age, width = 5, height = 5, dpi = 300)
ggsave(paste(resdir_gd_fig, 'bam_smes_sml_net_pds_on_SAexp_rm_outliers.svg', sep = ''), plot = plot_smes_pds, width = 5, height = 5, dpi = 300)
...
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Plotting of effects of mean GDs per network range length (predicted marginal trajectories) on S-A axis expansion -> one plot per category (s,m,l)
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
short_grid<- seq(
  min(mean_gd_short_net, na.rm=TRUE),
  max(mean_gd_short_net, na.rm=TRUE),
  length.out=200
)

newdata_short <- expand.grid(
  age = mean(covar_df$age, na.rm = TRUE),  # hold age at mean
  sex = levels(covar_df$sex),
  pds = mean(covar_df$pds, na.rm = TRUE),  # hold PDS at mean
  tot_SA = mean(covar_df$tot_SA, na.rm = TRUE),  # hold total SA at mean
  mean_gd_short_net = short_grid,  # these are the predictions that we're plotting
  mean_gd_medium_net = mean(mean_gd_medium_net, na.rm = TRUE),  # hold at mean
  mean_gd_long_net = mean(mean_gd_long_net, na.rm = TRUE),  # hold at mean
  site_id = covar_df$site_id[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family = covar_df$site.family[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family.subject = covar_df$site.family.subject[1]  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
)
...
# 2. Generate the predictions
smooth_preds_short <- fitted_values(
  bam_SA_exp_GDs_sml_net, 
  data = newdata_short, 
  exclude = c("s(site_id)", "s(site.family)", "s(site.family.subject)"),
  scale = "response"
)
...
# 3. Plot marginal trajectories
plot_margtraj_short <- ggplot(
  smooth_preds_short, 
  aes(x = mean_gd_short_net, y = .fitted, color = sex, fill = sex)
) +
  geom_ribbon(
    aes(ymin = .lower_ci, ymax = .upper_ci),  # Use .lower and .upper (instead of manual +/- 2*se) in order to get the limits of the credible interval on the fitted values, on the specified scale
    alpha = 0.25, 
  ...
  labs(x = "Mean length of short-range network\nfunctional connections", y = "S-A axis expansion", color = "Sex", fill = "Sex") +
  theme_minimal(base_size = 14) +
  theme(
    axis.text = element_text(color = "black"),
    axis.title.y.right = element_text(angle = 90, vjust = 0.5)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_short_net_GDs_on_SAexp_rm_outliers.svg', sep = ''), plot = plot_margtraj_short, width = 6, height = 5, dpi = 300)
...
medium_grid<- seq(
  min(mean_gd_medium_net, na.rm=TRUE),
  max(mean_gd_medium_net, na.rm=TRUE),
  length.out=200
)

newdata_medium <- expand.grid(
  age = mean(covar_df$age, na.rm = TRUE),  # hold age at mean
  sex = levels(covar_df$sex),
  pds = mean(covar_df$pds, na.rm = TRUE),  # hold PDS at mean
  tot_SA = mean(covar_df$tot_SA, na.rm = TRUE),  # hold total SA at mean
  mean_gd_short_net = mean(mean_gd_short_net, na.rm = TRUE),  # hold at mean
  mean_gd_medium_net = medium_grid,  # these are the predictions that we're plotting
  mean_gd_long_net = mean(mean_gd_long_net, na.rm = TRUE),  # hold at mean
  site_id = covar_df$site_id[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family = covar_df$site.family[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family.subject = covar_df$site.family.subject[1]  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
...
# 2. Generate the predictions
smooth_preds_medium <- fitted_values(
  bam_SA_exp_GDs_sml_net, 
  data = newdata_medium, 
  exclude = c("s(site_id)", "s(site.family)", "s(site.family.subject)"),
  scale = "response"
...
# 3. Plot marginal trajectories
plot_margtraj_medium <- ggplot(
  smooth_preds_medium, 
  aes(x = mean_gd_medium_net, y = .fitted, color = sex, fill = sex)
) +
  geom_ribbon(
    aes(ymin = .lower_ci, ymax = .upper_ci),  # Use .lower and .upper (instead of manual +/- 2*se) in order to get the limits of the credible interval on the fitted values, on the specified scale
    alpha = 0.25, 
  ...
  labs(x = "Mean length of medium-range network\nfunctional connections", y = "S-A axis expansion", color = "Sex", fill = "Sex") +
  theme_minimal(base_size = 14) +
  theme(
    axis.text = element_text(color = "black"),
    axis.title.y.right = element_text(angle = 90, vjust = 0.5)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_medium_net_GDs_on_SAexp_rm_outliers.svg', sep = ''), plot = plot_margtraj_medium, width = 6, height = 5, dpi = 300)
...
long_grid<- seq(
  min(mean_gd_long_net, na.rm=TRUE),
  max(mean_gd_long_net, na.rm=TRUE),
  length.out=200
)

newdata_long <- expand.grid(
  age = mean(covar_df$age, na.rm = TRUE),  # hold age at mean
  sex = levels(covar_df$sex),
  pds = mean(covar_df$pds, na.rm = TRUE),  # hold PDS at mean
  tot_SA = mean(covar_df$tot_SA, na.rm = TRUE),  # hold total SA at mean
  mean_gd_short_net = mean(mean_gd_long_net, na.rm = TRUE),  # hold at mean
  mean_gd_medium_net = mean(mean_gd_medium_net, na.rm = TRUE),  # hold at mean
  mean_gd_long_net = long_grid,   # these are the predictions that we're plotting
  site_id = covar_df$site_id[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family = covar_df$site.family[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family.subject = covar_df$site.family.subject[1]  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
)
...
# 2. Generate the predictions
smooth_preds_long <- fitted_values(
  bam_SA_exp_GDs_sml_net, 
  data = newdata_long, 
  exclude = c("s(site_id)", "s(site.family)", "s(site.family.subject)"),
  scale = "response"
...
# 3. Plot marginal trajectories
plot_margtraj_long <- ggplot(
  smooth_preds_long, 
  aes(x = mean_gd_long_net, y = .fitted, color = sex, fill = sex)
) +
  geom_ribbon(
    aes(ymin = .lower_ci, ymax = .upper_ci),  # Use .lower and .upper (instead of manual +/- 2*se) in order to get the limits of the credible interval on the fitted values, on the specified scale
    alpha = 0.25, 
  ...
  labs(x = "Mean length of long-range network\nfunctional connections", y = "S-A axis expansion", color = "Sex", fill = "Sex") +
  theme_minimal(base_size = 14) +
  theme(
    axis.text = element_text(color = "black"),
    axis.title.y.right = element_text(angle = 90, vjust = 0.5)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_long_net_GDs_on_SAexp_rm_outliers.svg', sep = ''), plot = plot_margtraj_long, width = 6, height = 5, dpi = 300)
...
# Summarizing mean short/medium/long range connects (based on node (bins quantiles): short (0.33), medium (0.67), long (1))
...
bam_SA_exp_GDs_sml_node <- bam(
  SA_expansion_net_long ~
    sex +
    s(age, k = 10, fx = FALSE) +  # default
    s(pds, k = 10, fx = FALSE) +  # default
    s(mean_gd_short_node, k = 10, fx = FALSE) +  # default
    s(mean_gd_medium_node, k = 10, fx = FALSE) +  # default
    s(mean_gd_long_node, k = 10, fx = FALSE) +  # default
    tot_SA +
    s(site_id, bs = "re") +
    s(site.family, bs = "re") +
    s(site.family.subject, bs = "re"),
...
summary_bam_SA_exp_GDs_sml_node = summary(bam_SA_exp_GDs_sml_node)

# Save summary
capture.output(summary_bam_SA_exp_GDs_sml_node, file = file.path(resdir_gd, "summary_bam_SA_exp_GDs_sml_node_rm_outliers.txt"))

# Check for adequate basis dimension and model fit
mgcv::k.check(bam_SA_exp_GDs_sml_node)


# ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# Plotting of effects (on SA exp) of mean GDs per range length -> all in one
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
# Extract the data for more control
df_age <- smooth_estimates(bam_SA_exp_GDs_sml_node, select = "s(age)")
df_pds <- smooth_estimates(bam_SA_exp_GDs_sml_node, select = "s(pds)")

df_short <- smooth_estimates(bam_SA_exp_GDs_sml_node, select = "s(mean_gd_short_node)")
df_medium <- smooth_estimates(bam_SA_exp_GDs_sml_node, select = "s(mean_gd_medium_node)")
df_long <- smooth_estimates(bam_SA_exp_GDs_sml_node, select = "s(mean_gd_long_node)")


# Combine and set factor levels (by also renaming the predictor column to "mean_gd" for all networks before binding)

df_all_node <- bind_rows(
  df_short  %>% dplyr::rename(mean_gd = mean_gd_short_node)   %>% mutate(Length = "Short"),
  df_medium %>% dplyr::rename(mean_gd = mean_gd_medium_node)  %>% mutate(Length = "Medium"),
  df_long %>% dplyr::rename(mean_gd = mean_gd_long_node)  %>% mutate(Length = "Long")
) %>%
  mutate(Length = factor(Length, levels = connections_range_length))  # set legend order here


# Plot (effects of GDs of all networks on S-A axis expansion)
plot_smes_all_sml_node <- ggplot(df_all_node, aes(x = mean_gd, y = .estimate, color = Length, fill = Length)) +
  geom_ribbon(aes(ymin = .estimate - 2 * .se, ymax = .estimate + 2 * .se),
              alpha = 0.15, color = NA) +
  geom_line(linewidth = 1.2) +
  scale_color_manual(values = range_length_colors) +
...
plot_smes_all_sml_node


# before saving decide if you want with or without CIs (and remove or add "_CIs" accordingly)
ggsave(paste(resdir_gd_fig, 'bam_smes_sml_node_GDs_on_SAexp_rm_outliers_CIs.svg', sep = ''), plot = plot_smes_all_sml_node, width = 6, height = 5, dpi = 300)
...
ggsave(paste(resdir_gd_fig, 'bam_smes_sml_node_age_on_SAexp_rm_outliers.svg', sep = ''), plot = plot_smes_age, width = 5, height = 5, dpi = 300)
ggsave(paste(resdir_gd_fig, 'bam_smes_sml_node_pds_on_SAexp_rm_outliers.svg', sep = ''), plot = plot_smes_pds, width = 5, height = 5, dpi = 300)
...
short_grid<- seq(
  min(mean_gd_short_node, na.rm=TRUE),
  max(mean_gd_short_node, na.rm=TRUE),
  length.out=200
)
...
newdata_short <- expand.grid(
  ...
  pds = mean(covar_df$pds, na.rm = TRUE),  # hold PDS at mean
  tot_SA = mean(covar_df$tot_SA, na.rm = TRUE),  # hold total SA at mean
  mean_gd_short_node = short_grid,  # these are the predictions that we're plotting
  mean_gd_medium_node = mean(mean_gd_medium_node, na.rm = TRUE),  # hold at mean
  mean_gd_long_node = mean(mean_gd_long_node, na.rm = TRUE),  # hold at mean
  site_id = covar_df$site_id[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family = covar_df$site.family[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family.subject = covar_df$site.family.subject[1]  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
)
...
plot_margtraj_short <- ggplot(
  smooth_preds_short, 
  aes(x = mean_gd_short_node, y = .fitted, color = sex, fill = sex)
) +
  geom_ribbon(
    aes(ymin = .lower_ci, ymax = .upper_ci),  # Use .lower and .upper (instead of manual +/- 2*se) in order to get the limits of the credible interval on the fitted values, on the specified scale
    alpha = 0.25, 
  ...
  labs(x = "Mean length of short-range regional\nfunctional connections", y = "S-A axis expansion", color = "Sex", fill = "Sex") +
  theme_minimal(base_size = 14) +
  theme(
    axis.text = element_text(color = "black"),
    axis.title.y.right = element_text(angle = 90, vjust = 0.5)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_short_node_GDs_on_SAexp_rm_outliers.svg', sep = ''), plot = plot_margtraj_short, width = 6, height = 5, dpi = 300)
...
medium_grid<- seq(
  min(mean_gd_medium_node, na.rm=TRUE),
  max(mean_gd_medium_node, na.rm=TRUE),
  length.out=200
)
...
newdata_medium <- expand.grid(
  ...
  pds = mean(covar_df$pds, na.rm = TRUE),  # hold PDS at mean
  tot_SA = mean(covar_df$tot_SA, na.rm = TRUE),  # hold total SA at mean
  mean_gd_short_node = mean(mean_gd_short_node, na.rm = TRUE),  # hold at mean
  mean_gd_medium_node = medium_grid,  # these are the predictions that we're plotting
  mean_gd_long_node = mean(mean_gd_long_node, na.rm = TRUE),  # hold at mean
  site_id = covar_df$site_id[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family = covar_df$site.family[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family.subject = covar_df$site.family.subject[1]  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
...
plot_margtraj_medium <- ggplot(
  smooth_preds_medium, 
  aes(x = mean_gd_medium_node, y = .fitted, color = sex, fill = sex)
) +
  geom_ribbon(
    aes(ymin = .lower_ci, ymax = .upper_ci),  # Use .lower and .upper (instead of manual +/- 2*se) in order to get the limits of the credible interval on the fitted values, on the specified scale
    alpha = 0.25, 
  ...
  labs(x = "Mean length of medium-range regional\nfunctional connections", y = "S-A axis expansion", color = "Sex", fill = "Sex") +
  theme_minimal(base_size = 14) +
  theme(
    axis.text = element_text(color = "black"),
    axis.title.y.right = element_text(angle = 90, vjust = 0.5)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_medium_node_GDs_on_SAexp_rm_outliers.svg', sep = ''), plot = plot_margtraj_medium, width = 6, height = 5, dpi = 300)
...
long_grid<- seq(
  min(mean_gd_long_node, na.rm=TRUE),
  max(mean_gd_long_node, na.rm=TRUE),
  length.out=200
)
...
newdata_long <- expand.grid(
  ...
  pds = mean(covar_df$pds, na.rm = TRUE),  # hold PDS at mean
  tot_SA = mean(covar_df$tot_SA, na.rm = TRUE),  # hold total SA at mean
  mean_gd_short_node = mean(mean_gd_long_node, na.rm = TRUE),  # hold at mean
  mean_gd_medium_node = mean(mean_gd_medium_node, na.rm = TRUE),  # hold at mean
  mean_gd_long_node = long_grid,   # these are the predictions that we're plotting
  site_id = covar_df$site_id[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family = covar_df$site.family[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family.subject = covar_df$site.family.subject[1]  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
)
...
plot_margtraj_long <- ggplot(
  smooth_preds_long, 
  aes(x = mean_gd_long_node, y = .fitted, color = sex, fill = sex)
) +
  geom_ribbon(
    aes(ymin = .lower_ci, ymax = .upper_ci),  # Use .lower and .upper (instead of manual +/- 2*se) in order to get the limits of the credible interval on the fitted values, on the specified scale
    alpha = 0.25, 
  ...
  labs(x = "Mean length of long-range regional\nfunctional connections", y = "S-A axis expansion", color = "Sex", fill = "Sex") +
  theme_minimal(base_size = 14) +
  theme(
    axis.text = element_text(color = "black"),
    axis.title.y.right = element_text(angle = 90, vjust = 0.5)
...
ggsave(paste(resdir_gd_fig, 'bam_margtraj_long_node_GDs_on_SAexp_rm_outliers.svg', sep = ''), plot = plot_margtraj_long, width = 6, height = 5, dpi = 300)
