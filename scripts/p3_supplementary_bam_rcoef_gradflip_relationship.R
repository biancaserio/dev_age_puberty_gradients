...
# Checking relationship rcoef - gradient flip
...
resdir = '/data/p_02667/development/results/supplementary/rcoef_gradflip_relationship/'
resdir_fig = '/data/p_02667/development/results/supplementary/rcoef_gradflip_relationship/figures/'
...
#### WITHOUT age and pds in model

##### Main Model grad_flip ~ s(rcoef) + sex + tot_SA + random effect(site/family relatedness/subject)

bam_main_gflip <- bam(
  grad_flip ~ 
    sex +
    s(list_SA_axis_corr_to_ref_long_z, k = 10, fx = FALSE) +  # default
    tot_SA +
    s(site_id, bs = "re") +
    s(site.family, bs = "re") +
...
# Save summary
capture.output(summary_bam_main_gflip, file = file.path(resdir, "summary_bam_gflip_without_age_pds.txt"))
...
df_rcoef <- smooth_estimates(bam_main_gflip, select = "s(list_SA_axis_corr_to_ref_long_z)")

plot_smes_rcoef = ggplot(df_rcoef, aes(x = list_SA_axis_corr_to_ref_long_z, y = .estimate)) +
  geom_ribbon(aes(ymin = .estimate - 2 * .se, ymax = .estimate + 2 * .se),
              fill = "honeydew3", alpha = 0.25) +
  geom_line(color = "honeydew4", linewidth = 1.2) +
  labs(
    x = "Similarity to adult S-A axis (Fisher z)",
...
ggsave(paste(resdir_fig, 'bam_smes_rcoef_on_gradflip_without_age_pds.svg', sep = ''), plot = plot_smes_rcoef, width = 5, height = 5, dpi = 300)
...
rcoef_grid<- seq(
  min(list_SA_axis_corr_to_ref_long_z, na.rm=TRUE),
  max(list_SA_axis_corr_to_ref_long_z, na.rm=TRUE),
  length.out=200
)

newdata_rcoef <- expand.grid(
  list_SA_axis_corr_to_ref_long_z = rcoef_grid,
...
smooth_preds_rcoef <- fitted_values(
  bam_main_gflip, 
  data = newdata_rcoef, 
...
# 3. Plot marginal trajectories with dual X-axis
plot_margtraj_rcoef <- ggplot(
  smooth_preds_rcoef, 
  aes(x = list_SA_axis_corr_to_ref_long_z, y = .fitted, color = sex, fill = sex)
) +
  geom_ribbon(
  ...
  scale_color_manual(values = c("F" = "indianred3", "M" = "lightblue3")) +
  scale_fill_manual(values = c("F" = "indianred3", "M" = "lightblue3")) +
  
  # Secondary X-axis Configuration
  scale_x_continuous(
    sec.axis = sec_axis(
      trans = ~ ., # Keep spatial alignment 1:1
      name = "Equivalent r coefficient",
      labels = function(x) sprintf("%.2f", tanh(x)),
      breaks = derive() # Match ticks to the primary z-axis
    )
  ) +
  
  labs(
    x = "Similarity to adult S-A axis (Fisher z)", 
    y = "Probability of gradient flip", 
    color = "Sex", 
    fill = "Sex"
  ) +
  theme_minimal(base_size = 14) +
  theme(
    axis.text = element_text(color = "black"),
    # Ensures the top axis title doesn't hug the edge too tightly
    axis.title.x.top = element_text(margin = margin(b = 10))
  )


ggsave(paste(resdir_fig, 'bam_margtraj_rcoef_on_gradflip_without_age_pds.svg', sep = ''), plot = plot_margtraj_rcoef, width = 6, height = 5, dpi = 300)



#### WITH age and pds in model

##### Main Model grad_flip ~ s(rcoef) + sex + s(age) + s(pds) + tot_SA + random effect(site/family relatedness/subject)


bam_gflip_wAgePds <- bam(
  grad_flip ~ 
    sex +
    s(list_SA_axis_corr_to_ref_long_z, k = 10, fx = FALSE) +  # default
...
summary_bam_gflip_wAgePds <- summary(bam_gflip_wAgePds)

# Save summary
capture.output(summary_bam_gflip_wAgePds, file = file.path(resdir, "summary_bam_main_gflip_wAgePds.txt"))

# Check for adequate basis dimension and model fit
mgcv::k.check(bam_gflip_wAgePds)
...
df_rcoef <- smooth_estimates(bam_gflip_wAgePds, select = "s(list_SA_axis_corr_to_ref_long_z)")
df_age <- smooth_estimates(bam_gflip_wAgePds, select = "s(age)")
df_pds <- smooth_estimates(bam_gflip_wAgePds, select = "s(pds)")
...
ggsave(paste(resdir_fig, 'bam_smes_rcoef_on_gradflip_with_age_pds.svg', sep = ''), plot = plot_smes_rcoef, width = 5, height = 5, dpi = 300)
...
ggsave(paste(resdir_fig, 'bam_smes_age_on_gradflip_with_age_pds.svg', sep = ''), plot = plot_smes_age, width = 5, height = 5, dpi = 300)
ggsave(paste(resdir_fig, 'bam_smes_pds_on_gradflip_with_age_pds.svg', sep = ''), plot = plot_smes_pds, width = 5, height = 5, dpi = 300)
...
ggsave(paste(resdir_fig, 'bam_margtraj_rcoef_on_gradflip_with_age_pds.svg', sep = ''), plot = plot_margtraj_rcoef, width = 6, height = 5, dpi = 300)
...
newdata_age <- expand.grid(
  age = age_grid,
  sex = levels(covar_df$sex),
  list_SA_axis_corr_to_ref_long_z = mean(list_SA_axis_corr_to_ref_long_z, na.rm = TRUE),  # hold rcoef at mean
  pds = mean(covar_df$pds, na.rm = TRUE),  # hold PDS at mean
  tot_SA = mean(covar_df$tot_SA, na.rm = TRUE),  # hold total SA at mean
  site_id = covar_df$site_id[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
  site.family = covar_df$site.family[1],  # Use any existing level (e.g. the first level) - NA yields errors (levels will be ignored anyway)
...
ggsave(paste(resdir_fig, 'bam_margtraj_age_on_gradflip_with_age_pds.svg', sep = ''), plot = plot_margtraj_age, width = 6, height = 5, dpi = 300)
...
ggsave(paste(resdir_fig, 'bam_margtraj_pds_on_gradflip_with_age_pds.svg', sep = ''), plot = plot_margtraj_pds, width = 6, height = 5, dpi = 300)
